...
export default class PsHomeDashboard extends NavigationMixin(LightningElement) {
  ...
  handleDocuments() {
    this.dispatchEvent(
      new CustomEvent("navigate", { detail: { page: "documents" } })
    );

    // Uses an explicit relative URL rather than navigateToPage()'s named-page
    // lookup: this page's registered Experience Builder page name isn't
    // knowable from source, but its route path is fixed by its own
    // urlPrefix/parent route configuration.
    this[NavigationMixin.Navigate]({
      type: "standard__webPage",
      attributes: {
        url: "/partner-portal/onboarding-documents"
      }
    });
  }
